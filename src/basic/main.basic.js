...
function handleCalculateCartStuff() {
  ...
  if (subTot > 0) {
    for (let i = 0; i < cartItems.length; i++) {
      ...
      var qtyElem = cartItems[i].querySelector('.quantity-number');
      var q = Number.parseInt(qtyElem.textContent);
...
addBtn.addEventListener('click', () => {
  ...
  if (itemToAdd && itemToAdd.q > 0) {
    ...
    if (item) {
      ...
      var newQty = Number.parseInt(qtyElem['textContent']) + 1;
...
cartDisp.addEventListener('click', event => {
  ...
  if (
    ...
    if (tgt.classList.contains('quantity-change')) {
      ...
      var currentQty = Number.parseInt(qtyElem.textContent);
      ...
      var qtyElem = itemElem.querySelector('.quantity-number');
      var remQty = Number.parseInt(qtyElem.textContent);
      prod.q += remQty;
      itemElem.remove();
    }
    if (prod && prod.q < 5) {
